...
namespace SplitIt.Shared.Company
{
    public partial class CompanyInternshipsSection : ComponentBase
    {
        ...
        {
            companyInternship.CompanyInternshipOfferingTransport = offer;
            StateHasChanged();
        }
        ...
        private async Task HandleFileSelectedForUploadInternshipAsCompany(InputFileChangeEventArgs e)
        {
            try
            {
                if (e.File == null)
                {
                    companyInternship.CompanyInternshipAttachmentFile = null;
                    return;
                }
                ...
                companyInternship.CompanyInternshipAttachmentFile = memoryStream.ToArray();
            }
            catch (Exception ex)
            {
        ...
        private async Task SaveInternshipAsCompany(bool publishInternship)
        {
            ...
            try
            {
                ...
                companyInternship.CompanyInternshipUploadedDate = DateTime.Now;
                companyInternship.CompanyInternshipStatus = publishInternship ? "Δημοσιευμένη" : "Μη Δημοσιευμένη";
        ...
        private async Task LoadUploadedInternshipsAsync()
        {
            UploadedInternships = await dbContext.CompanyInternships
                ...
                .OrderByDescending(i => i.CompanyInternshipUploadedDate)
                .ToListAsync();
        }
        ...
        private async Task ApplyFiltersAndUpdateCounts()
        {
            FilteredInternships = selectedStatusFilterForInternships == "Όλα"
                ...
                : UploadedInternships.Where(i => i.CompanyInternshipStatus == selectedStatusFilterForInternships).ToList();

            totalCount = UploadedInternships.Count;
            publishedCount = UploadedInternships.Count(i => i.CompanyInternshipStatus == "Δημοσιευμένη");
            unpublishedCount = UploadedInternships.Count(i => i.CompanyInternshipStatus == "Μη Δημοσιευμένη");
            withdrawnCount = UploadedInternships.Count(i => i.CompanyInternshipStatus == "Ανακληθείσα");
        ...
        private async Task ExecuteBulkStatusChangeForInternships(string newStatus)
        {
            ...
            foreach (var i in internshipsToUpdate)
                i.CompanyInternshipStatus = newStatus;
        ...
        private async Task ExecuteBulkCopyForInternships()
        {
            ...
            foreach (var i in internshipsToCopy)
            {
                var copy = new CompanyInternship
                {
                    ...
                    CompanyInternshipStatus = "Μη Δημοσιευμένη",
                    CompanyEmailUsedToUploadInternship = CurrentUserEmail,
                    CompanyInternshipUploadedDate = DateTime.Now,
        ...
        private async Task DownloadAttachmentForCompanyInternships(int internshipId)
        {
            ...
            if (internshipWithAttachment?.CompanyInternshipAttachmentFile != null)
            {
                await JS.InvokeVoidAsync("BlazorDownloadAttachmentPositionFile",
                    $"internship-attachment-{internshipId}.pdf", "application/pdf", internshipWithAttachment.CompanyInternshipAttachmentFile);
            }
        }
        ...
        private void EditInternshipDetails(CompanyInternship internship)
        {
            currentInternship = new CompanyInternship
            {
                ...
                CompanyInternshipStatus = internship.CompanyInternshipStatus,
        ...
        private async Task UpdateInternshipStatusAsCompany(int internshipId, string newStatus)
        {
            ...
            {
                internshipToUpdate.CompanyInternshipStatus = newStatus;
                await dbContext.SaveChangesAsync();
                await LoadUploadedInternshipsAsync();
                await ApplyFiltersAndUpdateCounts();
            }
        ...
        private async Task LoadInternshipApplicants(string internshipHashedId)
        {
            ...
            try
            {
                var applicants = await dbContext.InternshipApplicants
                    .Where(a => a.RNGForInternshipPosition_HashedAsUniqueID == internshipHashedId)
                    .OrderByDescending(a => a.ApplicationDate)
                    .ToListAsync();

                internshipApplicantsMap[internshipHashedId] = applicants;

                var acceptedCount = applicants.Count(a => a.InternshipApplicantStatus == "Αποδεκτή");
                ...
                if (internship != null)
                    availableSlotsPerInternship_ForCompanyInternship[internshipHashedId] = internship.CompanyInternshipOpenSlots - acceptedCount;
            }
            catch (Exception ex)
            {
        ...
        private async Task HandleFileUploadToEditCompanyInternshipAttachment(InputFileChangeEventArgs e)
        {
            ...
            try
            {
                ...
                using var memoryStream = new System.IO.MemoryStream();
                await e.File.OpenReadStream(maxFileSize).CopyToAsync(memoryStream);
                currentInternship.CompanyInternshipAttachmentFile = memoryStream.ToArray();
            }
            catch (Exception ex)
            {
        ...
        private async Task SaveEditedInternship()
        {
            ...
            if (internshipToUpdate != null)
            {
                ...
                if (currentInternship.CompanyInternshipAttachmentFile != null)
                    internshipToUpdate.CompanyInternshipAttachmentFile = currentInternship.CompanyInternshipAttachmentFile;
        ...
        {
            var student = await dbContext.Students.FirstOrDefaultAsync(s => s.StudentEmail == studentEmail);
            if (student?.StudentCV != null)
            {
                await JS.InvokeVoidAsync("BlazorDownloadAttachmentPositionFile",
                    $"student-cv-{student.StudentRegNumber}.pdf", "application/pdf", student.StudentCV);
            }
        }
    }
}
