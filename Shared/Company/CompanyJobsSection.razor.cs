...
namespace SplitIt.Shared.Company
{
    public partial class CompanyJobsSection : ComponentBase
    {
        ...
        {
            job.PositionOfferingTransport = offer;
            StateHasChanged();
        }
        ...
        private async Task HandleFileSelected(InputFileChangeEventArgs e)
        {
            try
            {
                if (e.File == null)
                {
                    job.PositionAttachmentFile = null;
                    return;
                }
                ...
                job.PositionAttachmentFile = memoryStream.ToArray();
            }
            catch (Exception ex)
            {
        ...
        private async Task DownloadAttachmentForCompanyJobs(int jobId)
        {
            ...
            if (jobWithAttachment?.PositionAttachmentFile != null)
            {
                await JS.InvokeVoidAsync("BlazorDownloadAttachmentPositionFile",
                    $"job-attachment-{jobId}.pdf", "application/pdf", jobWithAttachment.PositionAttachmentFile);
            }
        }
        ...
        private async Task LoadJobApplicants(string positionHashedId)
        {
            ...
            try
            {
                var applicants = await dbContext.JobApplicants
                    .Where(a => a.RNGForJobPosition_HashedAsUniqueID == positionHashedId)
                    .OrderByDescending(a => a.ApplicationDate)
                    .ToListAsync();

                jobApplicantsMap[positionHashedId] = applicants;

                var acceptedCount = applicants.Count(a => a.JobApplicantStatus == "Αποδεκτή");
        ...
        private async Task ExecuteBulkActionForApplicants()
        {
            ...
            var applicantsToUpdate = await dbContext.JobApplicants
                .Where(a => selectedApplicantIds.Contains(a.Id))
                .ToListAsync();

            foreach (var applicant in applicantsToUpdate)
            {
                applicant.JobApplicantStatus = pendingBulkActionForApplicants;
            }
        ...
        private async Task HandleFileUploadToEditCompanyJobAttachment(InputFileChangeEventArgs e)
        {
            ...
            try
            {
                ...
                using var memoryStream = new System.IO.MemoryStream();
                await e.File.OpenReadStream(maxFileSize).CopyToAsync(memoryStream);
                currentJob.PositionAttachmentFile = memoryStream.ToArray();
            }
            catch (Exception ex)
            {
        ...
        private async Task SaveEditedJob()
        {
            ...
            if (jobToUpdate != null)
            {
                ...
                if (currentJob.PositionAttachmentFile != null)
                    jobToUpdate.PositionAttachmentFile = currentJob.PositionAttachmentFile;
